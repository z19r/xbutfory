...
class CssTokenTest < ActiveSupport::TestCase
  ...
  def defined_tokens
    Dir[CSS_ROOT.join('tokens/*.css')]
      .flat_map { |file| File.read(file).scan(/(--[a-z0-9-]+)\s*:/).flatten }
      .to_set
  end
  ...
  test 'no CSS references an undefined custom property' do
    ...
    Dir[CSS_ROOT.join('**/*.css')].each do |file|
      File
        ...
        .scan(/var\(\s*(--[a-z0-9-]+)/)
        .flatten
        .each { |ref| missing[ref] << file unless known.include?(ref) }
    end
    ...
    assert missing.empty?,
           "Phantom CSS tokens (undefined in tokens/*.css):\n" +
             missing
               .map { |t, fs|
                 "  #{t} → #{fs.map { |f| File.basename(f) }.uniq.join(', ')}"
               }
               .join("\n")
  end
end
