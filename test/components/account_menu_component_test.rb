...
class AccountMenuComponentTest < ViewComponent::TestCase
  test 'renders avatar initial, handle and dropdown items' do
    ...
    assert_selector '.c-account-menu__label',
                    text: 'SIGNED IN AS @apt_4b',
                    visible: :all
    assert_selector "a.c-account-menu__item[href='/account']",
                    text: 'Account settings',
                    visible: :all
    assert_selector "a.c-account-menu__item[href='/account/submissions']",
                    text: 'Manage submissions',
                    visible: :all
    assert_selector "form[action='/sign_out'] button.c-account-menu__item--signout",
                    text: 'Sign out',
                    visible: :all
  end
  ...
  test 'is wired to the account-menu controller and starts closed' do
    ...
    assert_selector "[data-account-menu-target='dropdown'][hidden]",
                    visible: :all
