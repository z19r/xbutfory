...
class Webhooks::StripeControllerTest < ActionDispatch::IntegrationTest
  def payment
    @payment ||= Payment.create!(
      entry: entries(:two), user: users(:member), amount_cents: 199, stripe_session_id: 'cs_hook',
    )
  end
  ...
  def completed_event(session_id: 'cs_hook', payment_intent: 'pi_hook')
    ...
    Struct.new(:type, :data).new('checkout.session.completed', Struct.new(:object).new(object))
  end
  ...
  test 'a bad signature is rejected' do
    ...
    stub_method(Stripe::Webhook, :construct_event, raiser) do
      post webhooks_stripe_path, params: '{}', headers: { 'HTTP_STRIPE_SIGNATURE' => 'nope' }
    end
